...
RUN powershell Copy-Item -Path 'C:\vcpkg\packages\podofo_x64-windows\lib\*' -Destination 'C:\lib' -Recurse `
    ...
    && powershell Copy-Item -Path 'C:\vcpkg\packages\libxml2_x64-windows\lib\*' -Destination 'C:\lib' -Recurse -Force`
...
RUN powershell Invoke-WebRequest -Uri https://github.com/tesseract-ocr/tesseract/archive/refs/tags/5.1.0.tar.gz -OutFile C:\ocr.tar.gz `
    ...
    && mkdir build `
    && cd build `
    && cmake .. -G "Visual Studio 17 2022" -A x64 -DCMAKE_TOOLCHAIN_FILE=C:\vcpkg\scripts\buildsystems\vcpkg.cmake -DCMAKE_INSTALL_PREFIX:PATH="C:\\" CMAKE_MSVC_RUNTIME_LIBRARY="MultiThreaded$<$<CONFIG:Debug>:Debug>"  `
    && cmake --build . --config Release`
    && cmake --build . --config Release --target install `
    && cd .. `
    && cd ..
