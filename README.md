...
- **Llama.cpp Integration**: The SDK integrates seamlessly with llama.cpp, to allow the users of SDK to select a model of their choice and integrate within the workflow using `llama` wrapper provided in the SDK.

- **Build-in powerful IBM Granite 4.0 1B model**: This compact yet capable language model is designed for a wide range of natural language processing tasks, including text extraction, classification, question answering, summarization, and more. It runs via **llama.cpp** and is optimized for CPU inference on lower-end desktops without GPU acceleration.
