...
**The Why:** * **The C++20 Sweet Spot:** C++20 provides the exact foundational tools we need for safe pipelines (`std::span` for zero-copy parsing, `consteval` for hardcoded depth limits, and Concepts to enforce static polymorphism across our lazy-evaluation pipelines).
...
**The Why:** Virtual functions and type-erased pointers force the CPU into indirect branch prediction misses, block the compiler's inlining optimizer, and force hidden OS heap allocations. By passing domain objects directly into templated pipelines (e.g., `template <Document T> void export(T doc)`), the compiler perfectly resolves dependencies via Argument-Dependent Lookup (ADL) at compile time. This natively provides zero-cost lazy evaluation without the need for mutable state or type-erasure.
...
### [DESIGN-UNIFIED-AUDIT]

**The Rule:** The SDK must not distinguish internally between "logging" and "diagnostics." Instead, it must utilize a single, dependency-injected `Audit` mechanism (passed via the `ExecutionContext`). This interface must accept structured, raw C++ values (e.g., `std::source_location`, domain tags, variable tuples) and strictly avoid premature string serialization within the SDK boundary.
...
### [DESIGN-API-SAFETY-POLICY]

**The Rule:** The SDK must enforce internal API preconditions (e.g., array bounds, null-checks) using a class-level or pipeline-level template policy (e.g., `SafetyPolicy`), which must default to `strict`. For accessors subject to logic/boundary errors, the return type must dynamically morph based on this policy. 
* In `strict` mode, the function performs the check and returns a value-based error router (e.g., `expected<T, ErrorCategory>`).
* In `relaxed` mode, the check is completely erased from the compiler's view, and the function returns the raw type (`T`) for maximum hardware speed.

**The Why:** This eliminates API duplication (e.g., `at()` vs `[]`) and avoids the "API virus" of forcing monadic error handling onto fundamentally safe mathematical operations. Evaluators and debug builds receive total safety, while production deployments can opt into raw, zero-overhead execution speed. 

### [DESIGN-COMPILE-TIME-CONTRACTS]

**The Rule:** The SDK must enforce API preconditions using custom, strong-type wrappers (e.g., `docwire::not_null<T>`, `docwire::ranged<T>`) to shift logic checks to compile-time wherever mathematically possible. Standard library GSL is banned, as its enforcement relies on global macros that violate our dependency injection model.

**The Why:** Strong types make APIs self-documenting and mathematically un-compilable if used incorrectly. A compile-time error is infinitely cheaper than a runtime check, even a morphing template one.
...
### [API-COMPOSABILITY-PIPEABLE]

**The Rule:** Data transformation pipelines should provide composable Function Objects (Functors) that overload the pipe operator (`operator|`) for declarative, left-to-right composition (e.g., `input_span | docwire::zip::decompress | docwire::pdf::parse`). 
**The Clarification:** This does *not* mandate strict compatibility with the C++20 `<ranges>` library, as custom range adaptors in C++20 require excessive boilerplate and our parsers often transform singular data blocks rather than iterating sequences. We are adopting the ergonomic syntax of ranges, not necessarily the standard library iterator concepts.
**The Why:** Overloading `operator|` allows developers to elegantly chain operations without deeply nested function calls, drastically improving API readability while allowing the compiler to inline the entire chain into a single execution loop.

### [API-NAMING-PIPELINE_VERBS]

**The Rule:** Functors and pipeline adaptors intended for use with the pipe operator (`operator|`) must be named using lowercase action verbs (e.g., `parse`, `decompress`, `extract`), strictly avoiding Object-Oriented nouns (e.g., `Parser`, `Decompressor`).

**The Why:** Pipeline components represent data transformations, not stateful objects. Using verbs ensures the API reads fluently from left-to-right (`input | zip::decompress | pdf::parse`), perfectly matching the ergonomic standard set by C++20 `<ranges>`.
...
### [ERR-VALUE-BASED-ROUTING]

**The Rule:** Standard C++ exceptions (`throw`, `try`, `catch`) are strictly banned. All fallible algorithms must return a value-based result (e.g., `expected<T, error_category>`). The error payload must be a Trivially Copyable type (e.g., a lightweight enum or bitmask) that represents broad, coarse-grained control-flow states (e.g., `program_logic`, `uninterpretable_data`, `network_failure`). 

**The Why:** Maintaining hundreds of highly specific error codes creates maintenance hell for developers and is useless to the machine. Coarse-grained categories tell the caller exactly how to route the control flow (Abort, Retry, Skip) while keeping the return value register-fast and mathematically instantaneous on the CPU hot path.
...
### [ERR-LOGIC-VIOLATION-ROUTING]

**The Rule:** When an internal API contract is violated in `strict` mode, the SDK must not blindly crash (e.g., hardcoded `std::abort`), nor should it silently swallow the error. The SDK must immediately push a pristine, blameless diagnostic trace of the violation to the injected `Audit` interface, and then return the morphing `unexpected(ProgramLogic)` error code. 

**The Why:** Logic bugs represent host application errors, not SDK failures. By delegating the failure context to the `Audit` stream before returning the error, the SDK proves the mathematical impossibility of the user's request (exonerating the SDK from "crash blame"). This also grants the host application total control: their injected `Audit` implementation can optionally trigger a debug trap, throw an exception, or safely terminate the process on their own terms.
...
**The Why:** We rely on the C++ compiler's Dead Code Elimination to achieve O(1) teardown ([MEM-OPTIMIZER-DRIVEN-TEARDOWN]). Embedding OS synchronization primitives (which require complex destructors) inside arena-allocated data structures forces the compiler to abandon O(1) elision and execute a slow O(N) teardown. Furthermore, embedding locks inside allocators or data structures destroys deterministic O(1) cache performance during active parsing. Infrastructure manages threads; data pipelines just execute logic.
