...
inline constexpr unsigned long unicode_max = 0x10FFFFul;

inline const char* const NAMED_ENTITIES[][2] =
{
	{ "AElig;", "Æ" },
	{ "Aacute;", "Á" },
	{ "Acirc;", "Â" },
...
inline int cmp(const void *key, const void *value)
{
	return strncmp((const char *)key, *(const char **)value,
		strlen(*(const char **)value));
}

inline const char *get_named_entity(const char *name)
{
...
inline size_t putc_utf8(unsigned long cp, char *buffer)
{
...
inline bool parse_entity(const char *current, char **to, const char **from)
{
...
inline size_t decode_html_entities_utf8(char* dest, const char* src)
{
	if(!src) src = dest;
	...
	for(const char *current; (current = strchr(from, '&'));)
	{
		...
		if(detail::parse_entity(current, &to, &from))
			continue;
