...
struct XLSParser::Implementation
{
	...
	void parseSharedStringTable(const std::vector<unsigned char>& sst_buf)
	{
		...
		if (sst_buf.size() < 8)
		{
			*m_log_stream << "Warning: error while parsing shared string table Buffer must contain at least 8 bytes.\n";
			return;
		}
	...
	void processRecord(int rec_type, const std::vector<unsigned char>& rec, std::string& text)
	{
		...
		switch (rec_type)
		{
			case XLS_BLANK:
			{
				if (rec.size() < 4)
				{
					*m_log_stream << "Warning: record is too short. XLS_BLANK must be 4 bytes in length\n";
					break;
				}
			...
			case XLS_FORMAT:
			{
				if (rec.size() < 2)
				{
					*m_log_stream << "Warning: record is too short. XLS_FORMAT must be 2 bytes in length\n";
					break;
				}
			...
			case XLS_FORMULA:
			{
				if (rec.size() < 14)
				{
					*m_log_stream << "Warning: record is too short. XLS_FORMULA must be 14 bytes in length\n";
					break;
				}
			...
			case XLS_INTEGER_CELL:
			{
				if (rec.size() < 9)
				{
					*m_log_stream << "Warning: record is too short. XLS_INTEGER_CELL must be 9 bytes in length\n";
					break;
				}
				int row = getU16LittleEndian(rec.begin());
				int col = getU16LittleEndian(rec.begin()+2);
			...
			case XLS_LABEL:
			{
				if (rec.size() < 6)
				{
					*m_log_stream << "Warning: record is too short. XLS_LABEL and XLS_RSTRING must be at least 6 bytes in length\n";
					break;
				}
				m_last_string_formula_row = -1;
				int row = getU16LittleEndian(rec.begin()); 
			...
			case XLS_LABEL_SST:
			{
				if (rec.size() < 8)
				{
					*m_log_stream << "Warning: record is too short. XLS_LABEL_SST must be at least 8 bytes in length\n";
					break;
				}
				m_last_string_formula_row = -1;
				int row = getU16LittleEndian(rec.begin()); 
			...
			case XLS_MULBLANK:
			{
				if (rec.size() < 4)
				{
					*m_log_stream << "Warning: record is too short. XLS_MULBLANK must be at least 4 bytes in length\n";
					break;
				}
				int row = getU16LittleEndian(rec.begin());
			...
			case XLS_MULRK:
			{
				if (rec.size() < 4)
				{
					*m_log_stream << "Warning: record is too short. XLS_MULRK must be at least 4 bytes in length\n";
					break;
				}
				m_last_string_formula_row = -1;
				int row = getU16LittleEndian(rec.begin());
				...
				{
					*m_log_stream << "Warning: record is too short. The specified XLS_MULRK record must be at least " << min_size << " bytes in length\n";
					break;
				}
			...
			case 0x303:
			{
				if (rec.size() < 14)
				{
					*m_log_stream << "Warning: record is too short. XLS_NUMBER (or record of number 0x03, 0x103, 0x303) must be 14 bytes in length\n";
					break;
				}
				m_last_string_formula_row = -1;
				int row = getU16LittleEndian(rec.begin());
			...
			case XLS_RK:
			{
				if (rec.size() < 10)
				{
					*m_log_stream << "Warning: record is too short. XLS_RK must be 10 bytes in length\n";
					break;
				}
				m_last_string_formula_row = -1;
				int row = getU16LittleEndian(rec.begin());
			...
			case 0x43:
			{
				if (rec.size() < 4)
				{
					*m_log_stream << "Warning: record is too short. XLS_XF (or record of number 0x43) must be 4 bytes in length\n";
					break;
				}
				XFRecord xf_record;
	...
	void parseXLS(ThreadSafeOLEStreamReader& reader, std::string& text)
	{
		...
		while (read_status)
		{
			...
			if (!reader.readU16(rec_type))
			{
				...
				*m_log_stream << "Error while parsing XLS, type of record could not be read. OLE Reader has reported an error: " << reader.getLastError() << "\n";
				break;
			}
			if (oleEof(reader))
			{
			...
			if (!reader.readU16(rec_len))
			{
				...
				*m_log_stream << "Error while parsing XLS, length of record could not be read. OLE Reader has reported an error: " << reader.getLastError() << "\n";
				break;
			}
			if (rec_len > 0)
			{
				...
				if (!read_status)
					*m_log_stream << "Error while reading next record. OLE Reader has reported an error: " << reader.getLastError() << "\n";
			}
			else
				rec.clear();
			if (eof_rec_found)
